      ...
      <div className="h-full max-w-7xl mx-auto flex flex-col min-h-0">
        ...
        <div className="flex gap-4 overflow-x-auto overflow-y-visible py-2 px-1 snap-x snap-mandatory no-scrollbar flex-1 min-h-0 items-center">
          {certificationsData.map((item, idx) => (
            ...
            >
              <div className="p-4 sm:p-5 rounded-2xl border border-gray-200 dark:border-neutral-800 bg-white/80 dark:bg-black/60 backdrop-blur-xl shadow-lg min-h-[320px] sm:min-h-[370px] flex flex-col group">
                ...
                <h3 className="shrink-0 text-sm sm:text-base font-semibold text-black dark:text-white leading-tight mb-1 line-clamp-2 text-center">
                  {item.title}
                </h3>
                <p className="shrink-0 text-xs sm:text-sm font-bold text-gray-500 dark:text-gray-400 mb-3 text-center">
                  {item.issuer}
                </p>
                <div className="mt-auto w-full aspect-video rounded-xl overflow-hidden border border-gray-100 dark:border-neutral-800 bg-gray-50 dark:bg-neutral-900 shrink-0">
                  <img
                    src={item.image}
                    alt={item.title}
                    loading="lazy"
