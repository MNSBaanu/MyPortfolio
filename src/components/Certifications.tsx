        ...
        <div className="flex gap-4 overflow-x-auto py-6 px-4 sm:px-6 snap-x snap-mandatory no-scrollbar shrink-0">
          {certificationsData.map((item, idx) => (
            <div key={idx} className="snap-start flex-shrink-0 w-[240px] xs:w-[260px] sm:w-[280px]">
              <div className="p-4 rounded-2xl border border-gray-200 dark:border-neutral-800 bg-white/80 dark:bg-black/60 backdrop-blur-xl shadow-lg h-[320px] flex flex-col">
                ...
                <h3 className="text-sm font-semibold text-black dark:text-white leading-tight mb-1 line-clamp-2">{item.title}</h3>
                <p className="text-xs font-bold text-gray-500 dark:text-gray-400 mb-3">{item.issuer}</p>
            ...
            >
              {certificationsData.map((item, idx) => (
                <motion.div key={idx} initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: idx * 0.1 }} viewport={{ once: true }}
                  className="w-[280px] flex-shrink-0 p-4 rounded-2xl border border-gray-200 dark:border-neutral-800 bg-white/80 dark:bg-black/60 backdrop-blur-xl shadow-lg h-[370px] flex flex-col group">
                  <div className="flex flex-col gap-1 mb-3">
                    <span className="text-[9px] font-black uppercase text-gray-400 tracking-widest">Cert</span>
                    <span className="text-[9px] font-bold bg-black dark:bg-white text-white dark:text-black px-2.5 py-0.5 rounded-full self-start whitespace-nowrap">{item.date}</span>
                  </div>
                  <h3 className="text-base font-semibold text-black dark:text-white leading-tight mb-1.5 line-clamp-2">{item.title}</h3>
                  <p className="text-sm font-bold text-gray-500 dark:text-gray-400 mb-3">{item.issuer}</p>
                  <div className="mt-auto w-full aspect-video rounded-xl overflow-hidden border border-gray-100 dark:border-neutral-800 bg-gray-50 dark:bg-neutral-900 group">
                    <img src={item.image} alt={item.title} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110"
                      onError={(e) => { e.currentTarget.src = 'https://placehold.co/400x400/111111/ffffff?text=CERTIFICATE' }} />
                  </div>
                </motion.div>
              ))}
            </motion.div>
