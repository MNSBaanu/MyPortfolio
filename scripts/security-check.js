...
// Helper to get all headers across all routes
const allVercelHeaders = vercelConfig.headers?.flatMap(route => route.headers || []) || [];

// Check 5: HTTPS enforcement
console.log('\n5. Checking HTTPS enforcement...');
const hasHSTS = allVercelHeaders.some(h => h.key === 'Strict-Transport-Security');
if (hasHSTS) {
...
const hasCSP = allVercelHeaders.some(h => h.key === 'Content-Security-Policy');
if (hasCSP) {
